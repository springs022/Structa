...
AUTO_FIXED_PIECES を1にすると、開始局面と指定局面で同じ位置にある駒について、指定手数内に動かして戻すことも別駒で置き換えることもできないかを一対一割当の下界で証明します。証明できた駒は既存の不動駒と同様に探索から除外します。比較検証などで無効にする場合は0を指定します。設定がない場合は1として扱います。OUTPUT_LEVEL が2以上の場合は、実行画面と結果ファイルに「自動設定不動駒：後手51玉、先手59玉」の形式で表示します。
