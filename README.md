...
# 並列プロセス数（0：自動＝物理コア数、1：直列実行）
PROCESSES = 0

# 終端の逆算手数（AUTO：自動、0：逆算なし、1～2：手動指定）
RETRO_PLIES = AUTO
```
...
0 を指定すると物理コア数を自動で使います。1 を指定すると従来どおりの直列実行になります。  
TT_MEMORY_MB は全体の上限として扱われ、各プロセスに等分されます（1 プロセスあたり最低 16MB）。

RETRO_PLIES で「終端の逆算手数」を指定します。標準の AUTO では、
10 手以下は逆算せず、11 手以上ではまず 1 手逆算します。1 手目が重ければ逆算を使わず、
1 手目が軽い場合だけ 2 手目も試します。2 手目が重ければ 1 手逆算に戻ります。  
0 は逆算なし、1・2 は手動指定です。  

なお、並列実行時に Ctrl+C で中断した場合、再開用ファイルに記録されるのは
「先頭から連続して検討が完了した初手の数」です。
...
| -- | -------------------------------------------- | ---: | --------: | ------: |
| 1   | 高坂研／Problem Paradise／2017年4月／ツイン a） | 9  | なし | 13秒 |
| 2   | 高坂研／Problem Paradise／2017年4月／ツイン b） | 9  | なし | 14秒 |
| 3   | Proof Tachioka／Problem Paradise／2020年7月    | 10 | なし | 12秒 |
| 4   | 竹野龍騎／Web Fairy Paradise／2008年8月         | 10 | なし | 5秒 |
| 5-1 | 松さか子／詰将棋パラダイス／1999年9月            | 11 | なし | 13分56秒 |
| 5-2 | 松さか子／詰将棋パラダイス／1999年9月            | 11 | 18枚 | 35秒 |
| 6   | 神本多聞／詰将棋パラダイス／2020年8月            | 11 | なし | 22秒 |
| 7   | 高坂研／Problem Paradise／2017年4月            | 12 | なし | 0秒 |
| 8   | 宮内章良／詰将棋パラダイス／2012年3月            | 12 | 18枚 | 18分27秒 |
| 9   | 藤原俊雅／詰将棋パラダイス／2020年2月            | 14 | なし | 26分59秒 |
| 10  | 藤原勝博／詰将棋パラダイス／2025年8月            | 15 | なし | 1分51秒 |
